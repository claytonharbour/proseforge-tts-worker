...
func main() {
	...
	textPath := flag.String("text", "", "Source text file for phoneme-source diagnosis (no audio/whisper needed)")
	dataDir := flag.String("data-dir", "data", "Directory holding phonemizer data files (for -text mode)")
	flag.Parse()

	// Text mode: phoneme-source breakdown only — no audio, no whisper.
	// Reports which dictionary layer (custom/homograph/gold/silver/lts/skip)
	// resolves each word, plus the LTS-fallback inventory. Use to pre-flight
	// a section for pronunciation risk before narrating.
	if *textPath != "" {
		runTextDiagnosis(*textPath, *dataDir, *jsonOutput)
		return
	}
...
// runTextDiagnosis runs the phonemizer's per-word source diagnosis over a text
// file and reports the dictionary-layer breakdown plus the LTS-fallback and
// skip inventories. No audio or whisper involved — pure phonemization path.
func runTextDiagnosis(textPath, dataDir string, jsonOutput bool) {
	...
	ph, err := phonemizer.New(phonemizer.Config{
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error building phonemizer (check -data-dir=%q): %v\n", dataDir, err)
		os.Exit(1)
	}

	results := ph.DiagnoseText(string(data))

	// Aggregate: counts by source, and unique words per noteworthy source.
	counts := map[string]int{}
	seen := map[string]bool{}
	uniqueBySource := map[string][]phonemizer.WordDiagnosis{}
	for _, r := range results {
		counts[r.Source]++
		key := r.Source + "\x00" + strings.ToLower(r.Word)
		if seen[key] {
			continue
		}
		seen[key] = true
		uniqueBySource[r.Source] = append(uniqueBySource[r.Source], r)
	}

	for _, list := range uniqueBySource {
	...
	if jsonOutput {
		out := map[string]interface{}{
			...
			"counts":          counts,
			"lts_inventory":   uniqueBySource["lts"],
			"skip_inventory":  uniqueBySource["skip"],
			"silver_inventory": uniqueBySource["silver"],
		}
		enc := json.NewEncoder(os.Stdout)
		enc.SetIndent("", "  ")
		enc.Encode(out)
	...
	order := []string{"custom", "homograph", "gold", "silver", "lts", "skip", "builtin"}
	fmt.Printf("=== Phoneme Source Diagnosis ===\n")
	fmt.Printf("Text: %s (%d words)\n\n", filepath.Base(textPath), len(results))
	...
	// LTS-fallback and skip are the pronunciation-risk surfaces — list them.
	for _, src := range []string{"lts", "skip"} {
		list := uniqueBySource[src]
		if len(list) == 0 {
			continue
		}
		label := "LTS-fallback (pronunciation-risk)"
		if src == "skip" {
			label = "SKIPPED (no phonemes produced)"
		}
		fmt.Printf("\n%s — %d unique:\n", label, len(list))
		for _, w := range list {
			fmt.Printf("  %-20s %s\n", w.Word, w.Phonemes)
