...
func TestNormalizeAcronymAMPM(t *testing.T) {
	tests := []struct {
		input string
		want  string
	}{
		{"AM", "ay em"},   // meridiem → letters, NOT lowercased to the verb "am"
		{"PM", "pee em"},  // symmetric with AM
		{"am", "am"},      // lowercase verb is never matched by the acronym regex
		{"AM and FM", "ay em and eff em"},
	}
