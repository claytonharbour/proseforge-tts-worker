	...
	for i, word := range words {
		...
		if source == "skip" {
			if ipa := p.lts.Phonemize(core); ipa != "" {
				// Mirror phonemizeWord's heuristic exactly: a capitalized word
				// ending in 'a' gets a final schwa (Mara → mˈɑɹə). Without this
				// the diagnosis reports a phoneme string the engine never emits,
				// producing false-positive "defects" for cap+a names.
				if isCapitalized(core) && strings.HasSuffix(lower, "a") && !strings.HasSuffix(ipa, "ə") {
					ipa += "ə"
				}
