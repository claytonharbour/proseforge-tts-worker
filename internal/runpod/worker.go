...
func (w *Worker) processJob(j *job) {
	...
	// Start heartbeat pings so RunPod knows we're alive during long jobs.
	if w.config.PingURL != "" {
...
func (w *Worker) ping() {
	req, err := http.NewRequest(http.MethodGet, w.config.PingURL, nil)
	if err != nil {
...
func (w *Worker) postResult(jobID string, body []byte) {
	url := strings.Replace(w.config.PostOutputURL, "$ID", jobID, 1)
	...
	if resp.StatusCode != http.StatusOK {
		...
		io.Copy(io.Discard, resp.Body)
		log.Printf("POST result for job %s: 200 OK (%.2f MB, %s)", jobID, payloadMB, elapsed.Round(time.Millisecond))
	}
}
